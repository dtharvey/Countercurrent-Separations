...
A binomial distribution has a well-defined mean and standard deviation. After $n$ steps, the mean, $\mu$, of a solute's distribution is $\mu = np$, and its standard deviation, $\sigma$, is $\sigma = \sqrt{n p q}$ where $p$ and $q$ have their usual meanings. Try this. Return to the previous tab and model the countercurrent extraction of a mixture of solute A, with $p_a = 0.2$, and solute B with $p = 0.8$, and estimate, after $100$ steps, the mean and standard deviation for each solute's distribution. Then, calculate their expected means and standard deviations using a binomial distribution. How do your results compare to each other?

#### Comparing a CCE to a LLE

If you have worked on the learning module that covers liquid-liquid extractions, then you may have asked yourself why this module uses $p$ and $q$ instead of $D$, the distribution ratio. We know from the LLE learning module that for any step---and in the LLE learning module we assigned this to the aqueous phase---the fraction of solute not extracted is

$$(Q_{aq})_1 = \frac{(\text{mol aq})_1}{(\text{mol aq})_0} = \frac{V_{aq}}{DV_{org} + V_{aq}}$$

If $V_{aq} = V_{org}$, which is the case for the countercurrent extractions in this learning module, this simplifies to
